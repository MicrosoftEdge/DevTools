...
{%- for post in site.static_files -%}{% if post.path contains 'explainers' %}{% if post.path contains '.md' %}{% assign names = post.path | split: "/" %}{% for subpath in names %}{% if forloop.index0 == 2 %}{% if subpath != "README.md" and subpath != "explainer-template.md" %}* [{{ subpath | handle }}](/DevTools{{ post.path | replace:'.md','' }}){% endif %}{% endif %}{% endfor %}{% endif %}{% endif %}{%- endfor -%}
